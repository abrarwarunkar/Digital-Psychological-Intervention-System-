"""
Train DistilBERT risk detector for chat messages.

Features:
    - Fine-tunes DistilBERT for sequence classification
    - Handles class imbalance (weighted loss)
    - Optimizes for high recall on 'emergency' class
    - MLflow tracking
    - Saves model and tokenizer

Usage:
    python train_risk_detector.py
"""
...
def compute_metrics(pred):
    ...
    # Calculate per-class recall (crucial for emergency class)
    ...
    for i, recall_score in enumerate(per_class_recall):
...
def main():
    print("="*60)
    print("RISK DETECTOR TRAINING (DistilBERT)")
    print("="*60)
    ...
    # MLflow tracking
    mlflow.set_experiment("risk-detector")
    
    with mlflow.start_run(run_name=f"distilbert_risk_{datetime.now().strftime('%Y%m%d_%H%M%S')}"):
        print("\nStarting training...")
        trainer.train()
        ...
        # Save model
        models_dir = script_dir.parent / 'models' / 'risk_detector'
